...
function numberOfCharacters(string) {
  ...
  let valoresUnicos = [...new Set(arrayString)];
  valoresUnicos.forEach(elemento => {
    elementoCantidad[elemento] = arrayString.filter((ele) => ele === elemento).length
  });
...
for(let i = numeroToString.length - 1; i>= 0; i--) {
  numeroAlReves = numeroAlReves + numeroToString[i];
};
